...
def scale_image(image,do_scale):
    if(do_scale):
        ...
        # return np.maximum(0,np.minimum(image*255.,255))
    else:
        return np.maximum(0,np.minimum(image,255))
def save_image(image, image_path, do_scale =True):
    """Save one image.
    ...
    image_path : string.
    """
#    scipy.misc.imsave(image_path, scale_image(image,do_scale))
    scipy.misc.toimage(scale_image(image,do_scale),cmin=0,cmax=255).save(image_path)
def save_images(images, size, image_path, do_scale =True):
    ...
    def merge(images, size):
        ...
        for idx, image in enumerate(images):
            i = idx % size[1]
            j = idx // size[1]
            img[j*h:j*h+h, i*w:i*w+w, :] = scale_image(image,do_scale)
        return img
